...
export default function HeroSection({ data }: HeroSectionProps) {
  ...
  // Log the slides to see what's happening
  console.log("HeroSection slides:", slides);

  // Get the current slide's data
  const currentSlide = slides[current];
  console.log("Current slide index:", current, "Current slide image:", currentSlide?.image);
    ...
    <section className="relative h-[100svh] " onMouseEnter={pauseAutoplay} onMouseLeave={resumeAutoplay}>
      ...
      <div className="relative h-full w-full overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.div
            key={`slide-${current}-${currentSlide?.image}`} // Add image URL to key to force re-render
            custom={direction}
          ...
          >
            {currentSlide?.image && (
              <>
                {console.log("Rendering image with src:", currentSlide.image)}
                {currentSlide.image.startsWith("https://") ? (
                  ...
                  <div className="absolute inset-0">
                    <img
                      ...
                      onError={(e) => {
                        console.error("Image load error for:", currentSlide.image);
